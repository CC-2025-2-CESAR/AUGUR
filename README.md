...
- **Renderização com sprites + tileset.** Sprite sheets direcionais HD (4 direções × idle/walk/cast + hurt + death) pro jogador (augur) e 4 inimigos (carniçal/vidente/arauto/oráculo), magias e projéteis de inimigo estáticos rotacionados pela velocidade do projétil, e chão como tilemap de grama/natureza com hash determinístico (mesmo tile sempre na mesma posição). Fallback automático pra primitivas Raylib se um PNG não estiver presente — não derruba o jogo.
